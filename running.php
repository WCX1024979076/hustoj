...
//			$contest_ok = false;
	...
	if (!$contest_ok) {
		...
		$view_errors .= "<td>";
		$view_errors .= "<form class=form-inline method=post action=running.php?cid=$cid>";
	...
	$result_login=pdo_query($sql_login,$cid,$user_id);
	if(count($result_login)==0)
	{
		$sql_login="INSERT INTO `contest_login_time`(`contest_id`,`user_id`,`login_time`) values(?,?,?)";
		pdo_query($sql_login,$cid,$user_id,date('Y-m-d H:i:s', time()));
	}
	
	include("running_login_judge.php");
	//结束
  ...
  $sql = "SELECT * FROM `contest` WHERE `defunct`='N' AND  `training_id` is null AND `ftraining_date` is null ORDER BY `contest_id` DESC LIMIT 1000";

	if ($keyword) {
		$sql = "SELECT *  FROM contest WHERE contest.defunct='N' AND contest.title LIKE ? $wheremy AND  `training_id` is null AND `ftraining_date` is null ORDER BY contest_id DESC";
		$sql .= " limit ".strval($pstart).",".strval($pend); 

		$result = pdo_query($sql,$keyword);
	}
	else {
		$sql = "SELECT *  FROM contest WHERE contest.defunct='N' $wheremy AND `training_id` is null AND `ftraining_date` is null ORDER BY contest_id DESC";
		$sql .= " limit ".strval($pstart).",".strval($pend); 
		//echo $sql;
		$result = mysql_query_cache($sql);
	}
	...
	foreach ($result as $row) {
		...
		$view_contest[$i][1] = "<a href='running.php?cid=".$row['contest_id']."'>".$row['title']."</a>";
		$start_time = strtotime($row['start_time']);
		$end_time = strtotime($row['end_time']);
		$now = time();
