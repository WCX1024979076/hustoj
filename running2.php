...
//			$contest_ok = false;
	...
	if (!$contest_ok) {
		...
		$view_errors .= "<td>";
		$view_errors .= "<form class=form-inline method=post action=running2.php?cid=$cid>";
	...
	$sql_login="SELECT * FROM `contest_login_time` where `contest_id`=? AND `user_id`=?";
	$result_login=pdo_query($sql_login,$result[0]['training_id'],$user_id);
	if(count($result_login)==0&&!isset($_SESSION[$OJ_NAME.'_'.'administrator'])&&!isset($_SESSION[$OJ_NAME.'_'.'contest_creator']))
	{
		$view_errors = "<h3>训练赛未请假且未参加，无法参加补题赛。</h3>";
		require("template/".$OJ_TEMPLATE."/error.php");
		exit(0);
	}
  ...
  $sql = "SELECT * FROM `contest` WHERE `defunct`='N' AND `training_id` is not null AND `ftraining_date` is null ORDER BY `contest_id` DESC LIMIT 1000";

	if ($keyword) {
		$sql = "SELECT *  FROM contest WHERE contest.defunct='N' AND `training_id` is not null AND `ftraining_date` is null AND contest.title LIKE ? $wheremy  ORDER BY contest_id DESC";
		$sql .= " limit ".strval($pstart).",".strval($pend); 

		$result = pdo_query($sql,$keyword);
	}
	else {
		$sql = "SELECT *  FROM contest WHERE contest.defunct='N' $wheremy AND `training_id` is not null AND `ftraining_date` is null ORDER BY contest_id DESC";
		$sql .= " limit ".strval($pstart).",".strval($pend); 
		//echo $sql;
		$result = mysql_query_cache($sql);
	}
	...
	foreach ($result as $row) {
		...
		$view_contest[$i][1] = "<a href='running2.php?cid=".$row['contest_id']."'>".$row['title']."</a>";
		$start_time = strtotime($row['start_time']);
		$end_time = strtotime($row['end_time']);
		$now = time();
...
/////////////////////////Template
if (isset($_GET['cid']))
	require("template/".$OJ_TEMPLATE."/running.php");
else
	require("template/".$OJ_TEMPLATE."/runningset.php");
/////////////////////////Common foot
