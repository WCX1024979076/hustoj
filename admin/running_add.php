...
<?php 
  ...
  echo "<center><h3>训练赛添加</h3></center>";
...
if(isset($_POST['startdate'])){
   ...
   * 接收login_ip_limit
   */
  $login_ip_limit=$_POST['login_ip_limit'];
  if($login_ip_limit!=NULL)
    $login_ip_limit=1;
  else
    $login_ip_limit=0;
  //结束
  ...
  $sql = "INSERT INTO `contest`(`title`,`start_time`,`end_time`,`private`,`langmask`,`description`,`password`,`user_id`,`login_ip_limit`)
          VALUES(?,?,?,?,?,?,?,?,?)";
  ...
  $cid = pdo_query($sql,$title,$starttime,$endtime,$private,$langmask,$description,$password,$user_id,$login_ip_limit) ;
  ...
  if(isset($_GET['cid'])){
    ...
    $login_ip_limit=$row['login_ip_limit'];
...
?>
  ...
  <div class="container">
    ...
    <p align=left>
      ...
      <p align=left>
      <?php echo "是否开启登录IP限制"?>
        <input type=checkbox name="login_ip_limit" value="checked" <?php if($login_ip_limit) echo 'checked="checked"' ?> >
      </p>
