...
if(isset($_POST['startdate'])){
   ...
   * 修改人：王春祥
   * 修改时间：2021/1/19
   * 修改目的：后台获取训练赛时间和补题赛开始时间
   * 主要修改内容：增加post，注：ftraining 即为 finish training-补题赛
  */
  $training_length=intval($_POST['training_hour']).":".intval($_POST['training_minute']).":00"; //获取训练赛时长
  $ftraining_date=$_POST['ftraining_date']." ".intval($_POST['ftraining_hour']).":".intval($_POST['ftraining_minute']).":00"; //获取补题赛开始时间
  echo $training_length,$ftraining_date;
  //结束
 ...
 * 修改目的：修改sql语句
 */
  $sql = "UPDATE `contest` SET `title`=?,`description`=?,`start_time`=?,`end_time`=?,`private`=?,`langmask`=?,`password`=?,`training_length`=?,`ftraining_date`=? WHERE `contest_id`=?";
  //echo $sql;
  pdo_query($sql,$title,$description,$starttime,$endtime,$private,$langmask,$password,$training_length,$ftraining_date,$cid);
//结束
  ...
  $training_length=$row['training_length']; ///增加两个属性
  $ftraining_date=$row['ftraining_date'];
...
<div class="container">
  <form method=POST>
     ...
     * 主要修改的内容：增加两个input标签
     ...
     <p align=left>
      <?php echo "训练赛时长:"?>
      <?php for($i=1;$i<=8;$i++) echo "&nbsp"; ?>
      Hour: <input class=input-mini type=text name=training_hour size=2 value='<?php echo substr($training_length,0,2)?>' >&nbsp;
      Minute: <input class=input-mini type=text name=training_minute value='<?php echo substr($training_length,3,2)?>' size=2 >
    </p>

    <p align=left>
      <?php echo "补题赛开始于:"?>
      <?php for($i=1;$i<=5;$i++) echo "&nbsp"; ?>
      <input class=input-large type=date name='ftraining_date' value='<?php echo substr($ftraining_date,0,10)?>' size=4 >
      Hour: <input class=input-mini type=text name=ftraining_hour size=2 value='<?php echo substr($ftraining_date,11,2)?>'>&nbsp;
      Minute: <input class=input-mini type=text name=ftraining_minute value='<?php echo substr($ftraining_date,14,2)?>' size=2 >
    </p>
