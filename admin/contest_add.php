...
if(isset($_POST['startdate'])){
  ...
  */
   ...
   * 修改人：王春祥
   * 修改日期：2021/1/18
   * 修改目的：修改sql语句，提交训练赛时长和补题赛开始时间
   * 主要修改内容：修改sql语句
   */
  $sql = "INSERT INTO `contest`(`title`,`start_time`,`end_time`,`private`,`langmask`,`description`,`password`,`user_id`,`training_length`,`ftraining_date`)
          VALUES(?,?,?,?,?,?,?,?,?,?)";
  ...
  $cid = pdo_query($sql,$title,$starttime,$endtime,$private,$langmask,$description,$password,$user_id,$training_length,$ftraining_date) ;
...
?>
  ...
  <div class="container">
    ...
    <p align=left>
      ...
      Hour: <input class=input-mini type=text name=training_hour size=2 value=04 >&nbsp;
      Minute: <input class=input-mini type=text name=training_minute value=00 size=2 >
    </p>

    <p align=left>
      <?php echo "补题赛开始于:"?>
      <?php for($i=1;$i<=5;$i++) echo "&nbsp"; ?>
      <input class=input-large type=date name='ftraining_date' value='<?php echo date('Y').'-'. date('m').'-'.date('d')?>' size=4 >
      Hour: <input class=input-mini type=text name=ftraining_hour size=2 value=<?php echo (date('H')+4)%24?>>&nbsp;
      Minute: <input class=input-mini type=text name=ftraining_minute value=00 size=2 >
    </p>
